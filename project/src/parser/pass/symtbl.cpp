...
sem_symbol_table::sem_symbol_table() : sym_idx_(1), sym_map_(), father_(nullptr), children_() {}

sem_symbol_table::sem_symbol_table(sem_symbol_table *father)
    : sym_idx_(1), sym_map_(), father_(father), children_() {}

sem_symbol_table *sem_symbol_table::father() const { return father_; }

sem_symbol_table *sem_symbol_table::make_child() {
  std::unique_ptr<sem_symbol_table> child = std::make_unique<sem_symbol_table>(this);
...
simple_symbol *sem_symbol_table::find_symbol(const char *name, sem_symbol_table *father) {
  if (father == nullptr)
    return nullptr;
...
simple_symbol *sem_symbol_table::find_symbol(const char *name) {
  std::unordered_map<std::string, simple_symbol>::iterator fnd{};
...
const simple_symbol *sem_symbol_table::find_symbol(const char *name, sem_symbol_table *father) const {
  if (father == nullptr)
    return nullptr;
...
const simple_symbol *sem_symbol_table::find_symbol(const char *name) const {
  std::unordered_map<std::string, simple_symbol>::const_iterator fnd{};
  if (fnd = sym_map_.find(name); fnd == sym_map_.end()) {
    return find_symbol(name, father_);
  }
...
void sem_symbol_table::add_symbol(bool is_constexpr, const char *name, const std::list<sem_expression *> *dimensions,
...
simple_expression_result sem_symbol_table::get_left_value(sem_identifier *identifier,
...
void sem_symbol_table::print(std::ostream &os) const {
  ...
  print_indent += 1;
  for (const std::unique_ptr<sem_symbol_table> &symtbl : children_) {
    symtbl->print(os);
  }
}