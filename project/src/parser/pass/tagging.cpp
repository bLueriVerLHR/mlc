...
symbol_tagging::symbol_tagging(sem_context *ctx) : ctx_(ctx) {}

void symbol_tagging::visit(sem_operation *operation) { operation->simple_tagging(this, ctx_); }

void symbol_tagging::visit(sem_region *region) {
  ctx_->enter_block();
  for (sem_operation *operation : region->operations()) {
    operation->simple_tagging(this, ctx_);
  }
  ctx_->exit_block();
}

void sem_operation::simple_tagging(sem_visitor *visitor, sem_context *ctx) {}

void sem_declaration::simple_tagging(sem_visitor *visitor, sem_context *ctx) {
  if (dims_ != nullptr) {
...
void sem_param_declaration::simple_tagging(sem_visitor *visitor, sem_context *ctx) {
  if (dims_ != nullptr) {
    for (sem_expression *&expr : *dims_) {
      simple_expression_result &&result = calculate_constexpr(expr, *ctx);
      if (result.type == SER_TYPE::ERROR) {
...
void sem_function_definition::simple_tagging(sem_visitor *visitor, sem_context *ctx) {
...
void sem_plain_block::simple_tagging(sem_visitor *visitor, sem_context *ctx) {
  if (block_ != nullptr) {
